...
//interface for one day of forecast or how the selected day forecast will look like
...
//Converting weather code to emoji 
//Using .indexOf instead of .includes for better ES5 compatibility
function getIcon(code: number): string {
  if (code === 0) return "☀️";
  if (code >= 1 && code <= 3) return "☁️";
  if ([61, 63, 80].indexOf(code) !== -1) return "🌧️";
  if ([71, 73].indexOf(code) !== -1) return "🌨️";
  if ([95, 96].indexOf(code) !== -1) return "⛈️";
  return "❓";  //return question mark if the weather code is unknown
}

//Shows selected day's details in the main card
...
//Switch button between Celsius and Fahrenheit
...
//This loads the live forecast data from Open-Meteo API
...
//This shows the forecast as soon as the page is loaded
loadForecast();
